...
def parse_augs(augs: str) -> dict[str, float | int | bool]:
    """Parse augmentation parameters from a string into a dictionary."""
    if not augs:
        return {}

    def parse_value(val: str) -> float | int | bool | str:
        val_lower = val.lower()
        if val_lower == "true":
            return True
    ...
    try:
        return {key: parse_value(value) for pair in augs.split(",") for key, value in [pair.split("=")]}
    except ValueError:
        raise ValueError("Invalid augmentation format. Use 'key1=value1,key2=value2'")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--augmentations", help="Comma separated values containing augmentations e.g horitzontal_flip=True,zoom=0.3"
    )
    ...
    parser.add_argument(
        "--no-weights", action="store_true", help="Don't download and use any pretrained model weights, random init"
    )
    ...
    parser.add_argument(
        "--val-dir", help="Validation data (can be supplied if you do not want it taken from training data"
    )
    ...
    no_arg = ["augmentations", "cfg", "model", "no-weights", "preview"]
    arg_dict = vars(args)
